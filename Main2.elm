...
main =
    spreadsheet
        ...
        |> List.map checksumRow
        |> List.sum
        |> toString
        |> text


checksumRow : List Int -> Int
checksumRow row =
    case row of
        [] ->
            -99999
        ...
        n :: ns ->
            case checksumRowHelper n ns of
                Just sum ->
                    sum
...
checksumRowHelper : Int -> List Int -> Maybe Int
checksumRowHelper n row =
    case row of
        [] ->
            Nothing
        ...
        a :: rowRemainder ->
            if rem a n == 0 then
                Just (a // n)
            else if rem n a == 0 then
                Just (n // a)
            else
                checksumRowHelper n rowRemainder
...
main1 =
    spreadsheet
        ...
        |> List.map
            (\row ->
                Maybe.withDefault -999999 (List.maximum row)
                    - Maybe.withDefault -999999 (List.minimum row)
            )
        |> List.sum
        |> toString
        |> text
